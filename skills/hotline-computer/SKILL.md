...
- Browser tasks: `browser navigate`, then `browser text`. Use returned refs with `fill`, `select`, `check`, and `click_ref`. Take a fresh snapshot after navigation or a page change. `fill` takes `text` or `secret`, never both — omit whichever one you are not using rather than sending it empty. `text` (for example, `{"action":"fill","ref":"e3","text":"QA"}`) types that string; an explicit empty string clears the field. `secret` (for example, `{"action":"fill","ref":"e3","secret":"GITHUB.username"}`, more below) types one of the person's stored secrets by name. Date/time inputs use native formats; `select` accepts `values` for multiple choices. An action result includes the retained value and HTML validity. Verify the submitted result, not just the field entry.
...
- Native screens: `capture` supplies a screenshot and window-scoped accessibility nodes. Pass `window` (an id, or part of its title) to read one app instead of every window, which with a browser open is most of the answer; pass `region` `[x, y, width, height]` to see part of the screen at full size when text is too small to read. The line after the image says how image pixels map to the screen coordinates `input` takes. Use `input` for mouse/keyboard work and `windows` to focus or arrange apps. `scroll` takes `direction` (`up`, `down`, `left`, `right`) and `clicks`. `type` answers once the focused field has taken every character, so a click after it cannot land while a slow app is still catching up; use `paste` for long text. To place an app beside its observer, call `windows tile` with `primary_id` and `observer_id` from `windows list`; the result verifies work-area geometry and respects app minimum sizes. When accessibility is unavailable, use screenshot coordinates; never assume another window's tree belongs to this app.
...
`exec` waits up to 45 seconds; `command` alone may be a whole shell line (`"ls -la | head"`), or give a program and an `args` array. No call waits longer than 50 seconds, so its answer arrives before a client's one-minute tool timeout; `shell ready` or `shell wait` with the job ID waits again. Its answer holds the last 8 KiB of stdout and of stderr (`max_output` changes that) with each stream's full size, because a command says how it went at the end. When a stream was longer, `more` names the log that holds all of it: `grep` it for what you need, or page through everything with `shell read`. Use `start` for builds, servers, installers, or other long work; it returns a job ID promptly. `launch` uses the same job lifecycle for GUI apps. Supply an argv array, `cwd`, and optional `env`. Give every job a `label`: a short task in plain words (`Run the unit tests`, `Build the desktop app`, `Install Python 3.12`), because that is how a person watching the desktop's jobs list and terminal tells the jobs apart; the tools keep using the job ID. Use `request_id` when a start might be retried. A reused ID with different arguments is rejected.

- `shell read` accepts a byte `cursor` and returns `next_cursor`, output (32 KiB a page unless `max_output` says otherwise), state, and EOF. Keep the cursor when following output.

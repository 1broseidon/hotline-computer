...
`exec` waits up to 45 seconds; `command` alone may be a whole shell line (`"ls -la | head"`), or give a program and an `args` array. No call waits longer than 50 seconds, so its answer arrives before a client's one-minute tool timeout; `shell ready` or `shell wait` with the job ID waits again. Use `start` for builds, servers, installers, or other long work; it returns a job ID promptly. `launch` uses the same job lifecycle for GUI apps. Supply an argv array, `cwd`, and optional `env`. Give every job a `label`: a short task in plain words (`Run the unit tests`, `Build the desktop app`, `Install Python 3.12`), because that is how a person watching the desktop's jobs list and terminal tells the jobs apart; the tools keep using the job ID. Use `request_id` when a start might be retried. A reused ID with different arguments is rejected.
